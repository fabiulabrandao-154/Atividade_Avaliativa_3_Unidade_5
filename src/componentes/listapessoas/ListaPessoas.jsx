...
function ListaPessoas() {
  ...
  const pfDAORef = React.useRef(new PFDAO());
  const pjDAORef = React.useRef(new PJDAO());

  const carregarDados = React.useCallback(() => {
    let lista =
      tipo === "PF" ? pfDAORef.current.listar() : pjDAORef.current.listar();
  ...
  function excluirPessoa(id) {
    if (tipo === "PF") pfDAORef.current.excluir(id);
    else pjDAORef.current.excluir(id);
